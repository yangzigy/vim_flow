...
function! <SID>SrcExpl_WinNew(wincmd)
	...
	"echomsg "new window " . string(g:SrcExpl_winHeight) . a:wincmd
    ...
    exe ' rightbelow ' . string(g:SrcExpl_winHeight) . 'split '
...
function! <SID>SrcExpl_ViewOneDef(fpath, excmd)
    ...
    "endif
    call <SID>SrcExpl_WinEdit(fnamemodify(tagfiles()[0],':p:h'). '/' . a:fpath)
...
function! <SID>SrcExpl_Toggle()
    ...
    if s:SrcExpl_isRunning == 0
		...
		if g:SrcExpl_jumpKey != ""   " 直接映射跳转键，使用原代码机制出错
			exe "nnoremap " . g:SrcExpl_jumpKey .
				\ " :call g:SrcExpl_Jump()<CR>"
		endif
    else
